...
class AspectEn(models.Model):
    """When we detect some aspect on our comment, we save it on
    aspect term in the table aspect, in english language"""
...
class AspectEnAdmin(admin.ModelAdmin):
    display_list=["name","comment",'start_to','end_to']
    search_fields=["name","comment"]

admin.site.register(AspectEn,AspectEnAdmin)